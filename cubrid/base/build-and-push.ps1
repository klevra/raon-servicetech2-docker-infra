<#
.SYNOPSIS
  CUBRID 베이스 이미지 빌드 + servicetech2 레지스트리 push 스크립트 (PowerShell)

.DESCRIPTION
  지원 환경: Windows PowerShell 5.1+ / PowerShell 7+

  라이선스 주의사항
    - CUBRID는 완전 오픈소스(엔진 GPLv2 이상, 인터페이스/툴 BSD)이며 Docker Hub
      공식 이미지 사용에 별도 계정/라이선스 동의가 필요 없다 (Oracle EE와 달리
      로그인 불필요).
    - 운영(production) 환경 사용 금지. 테스트/개발/데모 전용.
    - CUBRID 컨테이너 실행(배포 단계)에는 --privileged 권한이 필요하다
      (11.4부터 공식 문서에 명시된 요구사항). 자세한 내용은 cubrid/deploy/deploy.ps1 참고.
#>
...
Write-Host "=============================================================="
Write-Host " CUBRID 베이스 이미지 빌드 + servicetech2 레지스트리 등록"
Write-Host "=============================================================="
...
Write-Host ""
Write-Host "CUBRID 버전을 선택하세요 (전부 계정/로그인 불필요, 공개 이미지):"
Write-Host "  1) latest  (최신 안정 버전)"
Write-Host "  2) 11.4    (--privileged 필수 버전)"
Write-Host "  3) 11.3    (구버전)"
$VerSel = Ask "번호 선택" "1"
switch ($VerSel) {
    "1" { $UpstreamImage = "cubrid/cubrid:latest"; $Tag = "latest" }
    "2" { $UpstreamImage = "cubrid/cubrid:11.4";   $Tag = "11.4" }
    "3" { $UpstreamImage = "cubrid/cubrid:11.3";   $Tag = "11.3" }
