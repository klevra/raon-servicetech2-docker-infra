#!/usr/bin/env bash
# ============================================================================
# CUBRID 베이스 이미지 빌드 + servicetech2 레지스트리 push 스크립트 (bash)
#
...
#     (11.4부터 공식 문서에 명시된 요구사항). 자세한 내용은 cubrid/deploy/deploy.sh 참고.
# ============================================================================
set -uo pipefail
...
echo "=============================================================="
echo " CUBRID 베이스 이미지 빌드 + servicetech2 레지스트리 등록"
echo "=============================================================="
...
# ---------- 2. CUBRID 버전 선택 ----------
echo
echo "CUBRID 버전을 선택하세요 (전부 계정/로그인 불필요, 공개 이미지):"
echo "  1) latest  (최신 안정 버전)"
echo "  2) 11.4    (--privileged 필수 버전)"
echo "  3) 11.3    (구버전)"
ask "번호 선택" "1"
case "$REPLY" in
  1) UPSTREAM_IMAGE="cubrid/cubrid:latest"; TAG="latest" ;;
  2) UPSTREAM_IMAGE="cubrid/cubrid:11.4";   TAG="11.4" ;;
  3) UPSTREAM_IMAGE="cubrid/cubrid:11.3";   TAG="11.3" ;;
