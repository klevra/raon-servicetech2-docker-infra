#!/usr/bin/env bash
# ============================================================================
# MariaDB 테스트 인스턴스 배포 스크립트 (bash) — servicetech2 레지스트리 기반
#
...
echo "=============================================================="
echo " MariaDB 테스트 인스턴스 배포 (servicetech2 레지스트리 기반)"
...
ask "번호 선택" "1"
if [[ "$REPLY" != "1" ]]; then
  err "현재는 MariaDB만 지원합니다."
  exit 1
fi
DB_KIND="mariadb"

# ---------- 2. MariaDB 버전(레지스트리 태그) ----------
echo
...
DEPLOY_IMAGE="servicetech2/mariadb-deploy:${TAG}"
...
# ---------- 3. 컨테이너 이름 ----------
echo
ask "컨테이너 이름" "mariadb-${TAG}-deploy"
...
info "(MariaDB 공식 이미지 기능으로 생성 — 별도 SQL 없이 지정한 DB에 자동으로 전체 권한 부여됨)"
APP_USER=""
APP_PASSWORD=""
APP_GENERATED_PW=0
...
if [[ -n "$DDL_DIR" || -n "$DML_DIR" ]]; then
  ...
  SETUP_MOUNT=1
  warn "MariaDB의 /docker-entrypoint-initdb.d/ 자동 실행은 데이터 볼륨이 비어있는 최초 기동 시에만 동작합니다 (이 프로젝트는 항상 휘발성이라 매번 최초 기동입니다)."
fi
...
echo " ---------------------------------------------------------"
echo " [관리자] 계정 : root"
echo " [관리자] URL  : jdbc:mariadb://localhost:${LISTENER_PORT}/${DB_NAME}"
if [[ -n "$APP_USER" ]]; then
  echo " ---------------------------------------------------------"
  echo " [앱]   계정   : $APP_USER  (${DB_NAME} DB에 전체 권한)"
  echo " [앱]   URL    : jdbc:mariadb://localhost:${LISTENER_PORT}/${DB_NAME}"
else
  echo " ---------------------------------------------------------"
  echo " [앱]   계정   : (생성 안 함, root로만 접속)"
fi
echo " ---------------------------------------------------------"
echo " 접속 IP 제한  : 없음 (0.0.0.0 바인딩, MARIADB_USER는 '%'(모든 host)로 생성됨)"
...
echo " Username   : root"
echo " JDBC URL   : jdbc:mariadb://localhost:${LISTENER_PORT}/${DB_NAME}"
if [[ "$GENERATED_PW" -eq 1 ]]; then
  echo " 접속 예시  : mariadb -h 127.0.0.1 -P ${LISTENER_PORT} -u root -p${DB_PASSWORD} ${DB_NAME}"
else
  echo " 접속 예시  : mariadb -h 127.0.0.1 -P ${LISTENER_PORT} -u root -p<입력한 비밀번호> ${DB_NAME}"
fi
if [[ -n "$APP_USER" ]]; then
  echo " ---------------------------- [앱] -----------------------------"
  ...
  if [[ "$APP_GENERATED_PW" -eq 1 ]]; then
    echo " 접속 예시  : mariadb -h 127.0.0.1 -P ${LISTENER_PORT} -u ${APP_USER} -p${APP_PASSWORD} ${DB_NAME}"
  else
    echo " 접속 예시  : mariadb -h 127.0.0.1 -P ${LISTENER_PORT} -u ${APP_USER} -p<입력한 비밀번호> ${DB_NAME}"
  fi
fi
echo "==========================================================="
} | log_tee
