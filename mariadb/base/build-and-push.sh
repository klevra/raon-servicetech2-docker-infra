#!/usr/bin/env bash
# ============================================================================
# MariaDB 베이스 이미지 빌드 + servicetech2 레지스트리 push 스크립트 (bash)
#
# 지원 환경 : Linux / macOS / Windows(Git Bash, WSL)
#
# 라이선스 주의사항
#   - MariaDB는 완전 오픈소스(GPLv2)이며 Docker Hub 공식 이미지 사용에
#     별도 계정/라이선스 동의가 필요 없다 (Oracle EE와 달리 로그인 불필요).
...
echo "=============================================================="
echo " MariaDB 베이스 이미지 빌드 + servicetech2 레지스트리 등록"
echo "=============================================================="
...
# ---------- 2. MariaDB 버전 선택 ----------
echo
echo "MariaDB 버전을 선택하세요 (전부 계정/로그인 불필요, 공개 이미지):"
echo "  1) latest  (최신 안정 버전)"
echo "  2) 11.4    (LTS)"
echo "  3) 10.11   (구버전 LTS, 레거시 호환용)"
ask "번호 선택" "1"
case "$REPLY" in
  1) UPSTREAM_IMAGE="mariadb:latest"; TAG="latest" ;;
  2) UPSTREAM_IMAGE="mariadb:11.4";   TAG="11.4" ;;
  3) UPSTREAM_IMAGE="mariadb:10.11";  TAG="10.11" ;;
...
# (원격 레지스트리는 일부 레이어 업로드가 타임아웃 나도 push 명령 자체는 성공으로 끝나는 경우가 있었음 — 2026-08-24 실제 발생, Oracle 작업 중 확인)
...
# 레지스트리 REST API(/v2/.../tags/list)를 직접 curl로 조회하는 방식으로 검증한다.
