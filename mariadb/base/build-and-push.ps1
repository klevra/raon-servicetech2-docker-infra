...
Write-Host "=============================================================="
Write-Host " MariaDB 베이스 이미지 빌드 + servicetech2 레지스트리 등록"
Write-Host "=============================================================="
...
Write-Host ""
Write-Host "MariaDB 버전을 선택하세요 (전부 계정/로그인 불필요, 공개 이미지):"
Write-Host "  1) latest  (최신 안정 버전)"
Write-Host "  2) 11.4    (LTS)"
Write-Host "  3) 10.11   (구버전 LTS, 레거시 호환용)"
$VerSel = Ask "번호 선택" "1"
switch ($VerSel) {
    "1" { $UpstreamImage = "mariadb:latest"; $Tag = "latest" }
    "2" { $UpstreamImage = "mariadb:11.4";   $Tag = "11.4" }
    "3" { $UpstreamImage = "mariadb:10.11";  $Tag = "10.11" }
