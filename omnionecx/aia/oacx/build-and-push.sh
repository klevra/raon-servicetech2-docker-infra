#!/usr/bin/env bash
# ============================================================================
# OmnioneCX oacx — OmnioneCX aia(2.0.0.1) 백포팅 트랙 전용 이미지 빌드 + push
#
...
if [[ ! -d "${SCRIPT_DIR}/app" ]]; then
  err "app/ 폴더가 없습니다: ${SCRIPT_DIR}/app (실제 oacx 산출물을 먼저 복사하세요)"
  exit 1
fi
JAR_COUNT=0
JAR_FILE=""
for f in "${SCRIPT_DIR}/app"/omnionecx-2.*.jar; do
  [[ -f "$f" ]] && { JAR_COUNT=$((JAR_COUNT + 1)); JAR_FILE="$f"; }
done
if [[ "$JAR_COUNT" -ne 1 ]]; then
  err "app/ 안에 omnionecx-2.*.jar 파일이 정확히 1개 있어야 합니다 (현재 ${JAR_COUNT}개)."
  exit 1
fi
...
SITE_TAG="$REPLY"
ask "oacx 실제 버전 번호 (레지스트리 태그로 쓰임, app/의 jar 파일 버전과 일치해야 함)" "$DEFAULT_VERSION"
VERSION="$REPLY"
