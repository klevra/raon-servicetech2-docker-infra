<#
.SYNOPSIS
  OmnioneCX oacx — OmnioneCX aia(2.0.0.1) 백포팅 트랙 전용 이미지 빌드 + push (PowerShell)

.DESCRIPTION
  전제조건: 이 스크립트와 같은 위치의 app\ 안에 실제 oacx 실행 산출물
            (omnionecx-2.*.jar, jdbc\ 등)이 있어야 한다. app\은 실제
            배포 산출물이라 git에는 커밋하지 않음 (.gitignore 확인).
  대상 베이스: jdk21/base로 이미 레지스트리에 등록된 servicetech2/jdk21 이미지
            (1.0.0.12/fsb/wooriib는 tomcat9-jdk8+WAR였지만, 이 oacx는 내장 Tomcat 포함
            Boot3 fat jar라 jdk21을 쓴다).

  주의: 레지스트리 태그는 jar 매니페스트의 Implementation-Version(2.0.1)
  그대로가 아니라 "2.0.0.1"을 쓴다 -- 이 트랙 자체를 2.0.0.1로 부르기로
  한 결정을 따름 (아래 버전 입력 시 기본값도 2.0.0.1).

  주의: site-tag는 1.0.0.12는 "latest", 2.0.0.3 트랙은 "2.0.0.3"을 전용으로 쓰고 있어서
  aia 트랙은 별도의 고정 이동 태그("aia")를 쓴다 -- latest/2.0.0.3과 충돌 금지.
#>
...
if (-not (Test-Path $AppDir -PathType Container)) {
    Write-Err2 "app 폴더가 없습니다: $AppDir (실제 oacx 산출물을 먼저 복사하세요)"
    exit 1
}
$JarFiles = Get-ChildItem -Path $AppDir -Filter "omnionecx-2.*.jar" -File -ErrorAction SilentlyContinue
if (-not $JarFiles -or $JarFiles.Count -ne 1) {
    $cnt = if ($JarFiles) { $JarFiles.Count } else { 0 }
    Write-Err2 "app 안에 omnionecx-2.*.jar 파일이 정확히 1개 있어야 합니다 (현재 ${cnt}개)."
    exit 1
}
$JarFile = $JarFiles[0]
...
$Version = Ask "oacx 실제 버전 번호 (레지스트리 태그로 쓰임, app의 jar 파일 버전과 일치해야 함)" $DefaultVersion
