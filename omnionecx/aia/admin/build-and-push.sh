#!/usr/bin/env bash
# ============================================================================
# OmnioneCX admin — OmnioneCX aia(2.0.0.1) 백포팅 트랙 전용 이미지 빌드 + push
#
...
# db/verifier/oacx/admin은 각각 하나의 리포지토리로 통합 관리된다.
#
...
IMAGE_KIND="omnionecx2-admin"
SITE="OmnioneCX aia(2.0.0.1) 백포팅 트랙"
DEFAULT_SITE_TAG="aia"
...
echo "=============================================================="
echo " OmnioneCX admin (${SITE}) 이미지 빌드 + 레지스트리 등록"
echo "=============================================================="
...
if [[ ! -d "${SCRIPT_DIR}/app" ]]; then
  err "app/ 폴더가 없습니다: ${SCRIPT_DIR}/app (실제 admin 산출물을 먼저 복사하세요)"
  exit 1
fi
JAR_COUNT=0
JAR_FILE=""
for f in "${SCRIPT_DIR}/app"/omnionecx-admin-2.*.jar; do
  [[ -f "$f" ]] && { JAR_COUNT=$((JAR_COUNT + 1)); JAR_FILE="$f"; }
done
if [[ "$JAR_COUNT" -ne 1 ]]; then
  err "app/ 안에 omnionecx-admin-2.*.jar 파일이 정확히 1개 있어야 합니다 (현재 ${JAR_COUNT}개)."
  exit 1
fi
...
# ============================================================================
echo
ask "사이트 코드 (이동 태그로 사용 -- 이 트랙이 현재 쓰는 버전을 가리킴)" "$DEFAULT_SITE_TAG"
SITE_TAG="$REPLY"
ask "admin 실제 버전 번호 (레지스트리 태그로 쓰임, app/의 jar 파일 버전과 일치해야 함)" "$DEFAULT_VERSION"
VERSION="$REPLY"
...
BASE_IMAGE="${LOCAL_REGISTRY}/${NAMESPACE}/jdk21:latest"
...
echo "              : $MOVING_IMAGE (이동 태그, 이 트랙이 현재 쓰는 버전)"
fi
echo "==========================================================="
}
...
if ! docker pull "$BASE_IMAGE"; then
  err "베이스 이미지 pull 실패. jdk21/base/build-and-push.sh 로 먼저 등록하세요."
  exit 1
fi
