<#
.SYNOPSIS
  OmnioneCX admin — OmnioneCX aia(2.0.0.1) 백포팅 트랙 전용 이미지 빌드 + push (PowerShell)

.DESCRIPTION
  전제조건: 이 스크립트와 같은 위치의 app\ 안에 실제 admin 실행 산출물
            (omnionecx-admin-2.*.jar, jdbc\ 등)이 있어야 한다. app\은 실제
            배포 산출물이라 git에는 커밋하지 않음 (.gitignore 확인).
  대상 베이스: jdk21/base로 이미 레지스트리에 등록된 servicetech2/jdk21 이미지.

  admin은 1.0.0.12/wooriib/fsb 트랙에는 없던 신규 컴포넌트다.

  주의: 레지스트리 태그는 jar 매니페스트의 Implementation-Version(2.0.1)
  그대로가 아니라 "2.0.0.1"을 쓴다 -- 이 트랙 자체를 2.0.0.1로 부르기로
  한 결정을 따름 (아래 버전 입력 시 기본값도 2.0.0.1). site-tag는
  1.0.0.12는 "latest", 2.0.0.3 트랙은 "2.0.0.3"을 쓰므로 aia 트랙은
  별도의 고정 이동 태그("aia")를 쓴다 -- 충돌 금지.
#>
...
$ImageKind = "omnionecx2-admin"
$Site = "OmnioneCX aia(2.0.0.1) 백포팅 트랙"
$DefaultSiteTag = "aia"
$DefaultVersion = "2.0.0.1"
...
Write-Host "=============================================================="
Write-Host " OmnioneCX admin ($Site) 이미지 빌드 + 레지스트리 등록"
Write-Host "=============================================================="
...
if (-not (Test-Path $AppDir -PathType Container)) {
    Write-Err2 "app 폴더가 없습니다: $AppDir (실제 admin 산출물을 먼저 복사하세요)"
    exit 1
}
$JarFiles = Get-ChildItem -Path $AppDir -Filter "omnionecx-admin-2.*.jar" -File -ErrorAction SilentlyContinue
if (-not $JarFiles -or $JarFiles.Count -ne 1) {
    $cnt = if ($JarFiles) { $JarFiles.Count } else { 0 }
    Write-Err2 "app 안에 omnionecx-admin-2.*.jar 파일이 정확히 1개 있어야 합니다 (현재 ${cnt}개)."
    exit 1
}
$JarFile = $JarFiles[0]
...
# ============================================================================
Write-Host ""
$SiteTag = Ask "사이트 코드 (이동 태그로 사용 -- 이 트랙이 현재 쓰는 버전을 가리킴)" $DefaultSiteTag
$Version = Ask "admin 실제 버전 번호 (레지스트리 태그로 쓰임, app의 jar 파일 버전과 일치해야 함)" $DefaultVersion
...
$BaseImage = "$LocalRegistry/$Namespace/jdk21:latest"
...
if ($Version -ne $SiteTag) {
    Write-Host "              : $MovingImage (이동 태그, 이 트랙이 현재 쓰는 버전)"
}
Write-Host "==========================================================="
...
if ($LASTEXITCODE -ne 0) { Write-Err2 "베이스 이미지 pull 실패. jdk21/base/build-and-push.ps1(.sh) 로 먼저 등록하세요."; exit 1 }
