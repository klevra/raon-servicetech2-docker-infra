<#
.SYNOPSIS
  OmnioneCX sample(테스트 페이지) — OmnioneCX aia(2.0.0.1) 백포팅 트랙 전용 이미지 빌드 + push (PowerShell)

.DESCRIPTION
  전제조건: 이 스크립트와 같은 위치의 app\ 안에 실제 sample 실행 산출물
            (mobileid-sample-springboot-1.0-SNAPSHOT.jar)이 있어야 한다.
            app\은 실제 배포 산출물이라 git에는 커밋하지 않음 (.gitignore 확인).
  대상 베이스: jdk21/base로 이미 레지스트리에 등록된 servicetech2/jdk21 이미지.

  sample은 1.0.0.12/wooriib/fsb 트랙에는 없던 신규 컴포넌트다 (oacx WAR에
  번들되어 있던 앱 호출 테스트 페이지가 2.0.0.3 계열에서는 별도 앱으로
  분리됨). aia의 jar는 2.0.0.3의 것과 MD5까지 동일 -- 버전 번호도 그대로
  "1.0-SNAPSHOT"을 유지한다.
#>
...
Write-Host "=============================================================="
Write-Host " OmnioneCX sample ($Site) 이미지 빌드 + 레지스트리 등록"
Write-Host "=============================================================="
...
if (-not (Test-Path $AppDir -PathType Container)) {
    Write-Err2 "app 폴더가 없습니다: $AppDir (실제 sample 산출물을 먼저 복사하세요)"
    exit 1
}
$JarFiles = Get-ChildItem -Path $AppDir -Filter "mobileid-sample-springboot-*.jar" -File -ErrorAction SilentlyContinue
if (-not $JarFiles -or $JarFiles.Count -ne 1) {
    $cnt = if ($JarFiles) { $JarFiles.Count } else { 0 }
    Write-Err2 "app 안에 mobileid-sample-springboot-*.jar 파일이 정확히 1개 있어야 합니다 (현재 ${cnt}개)."
    exit 1
}
$JarFile = $JarFiles[0]
...
$Version = Ask "sample 실제 버전 번호 (레지스트리 태그로 쓰임, app의 jar 파일 버전과 일치해야 함)" $DefaultVersion
