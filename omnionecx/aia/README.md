...
- [~] 모바일 지갑 앱으로 QR 스캔 → VP 제출 → 검증 흐름은 2.0.0.3 트랙
      기준으로는 `sandbox/`에서 구현·테스트 완료(운영측 제출/검증 확인).
      **단 aia의 oacx `2.0.0.1` 빌드에서 이 흐름에 문제가 있어 별도 추가
      검토 필요** (2.0.0.3 oacx `2.0.3`과 동일 계열이지만 빌드가 달라
      동작 차이 있음).
