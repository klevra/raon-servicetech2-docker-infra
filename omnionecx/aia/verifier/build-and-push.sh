#!/usr/bin/env bash
# ============================================================================
# OmnioneCX verifier — OmnioneCX aia(2.0.0.1) 백포팅 트랙 전용 이미지 빌드 + push
#
...
#           (1.0.0.12/fsb/wooriib는 jdk8이지만, 이 verifier는(Boot 2.7.4 jdk8 바이트코드라도) fat jar 실행형이라
#           jdk21을 쓴다).
#
# db/verifier/oacx/admin은 각각 하나의 리포지토리로 통합 관리된다. 태그는
# 실제 버전 번호를 쓰고, 사이트 코드(이동 태그)는 "이 트랙이 현재 쓰는
# 버전"을 가리키도록 매번 갱신된다.
#
...
IMAGE_KIND="omnionecx2-verifier"
SITE="OmnioneCX aia(2.0.0.1) 백포팅 트랙"
DEFAULT_SITE_TAG="aia"
DEFAULT_VERSION="1.3.36_fix"
