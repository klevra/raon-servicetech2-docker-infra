# ============================================================================
# OmnioneCX aia(2.0.0.1) 컨테이너 안으로 들어가는 명령어 (db/verifier/oacx/admin/sample 공용)
#
...
param(
    ...
    [ValidateSet("db", "verifier", "oacx", "admin", "sample")]
    [string]$Service,
...
if (Test-Path $EnvFile) {
    ...
    # PC에서 이 트랙을 여러 벌 띄우는 경우 등), 하드코딩하지 않고 그때
    # 저장해둔 값을 그대로 읽어 쓴다.
    $envLine = Select-String -Path $EnvFile -Pattern '^COMPOSE_PROJECT=' | Select-Object -First 1
    $ComposeProject = if ($envLine) { ($envLine.Line -split '=', 2)[1] } else { "omnionecx-aia" }
    ...
    docker compose -p omnionecx-aia exec $Service @Command
}
