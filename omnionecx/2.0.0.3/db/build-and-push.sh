#!/usr/bin/env bash
# ============================================================================
# OmnioneCX DB — OmnioneCX 2.0.0.3 트랙 전용 이미지 빌드 + servicetech2 레지스트리 push
#
# initdb/ 안의 DDL/DML은 이 트랙 전용 데이터를 이미 반영한 상태(git으로
# 추적, 민감정보 없음 -- 스키마와 초기 참조 데이터일 뿐).
# 대상 베이스: mariadb/base로 이미 레지스트리에 등록된 servicetech2/mariadb 이미지.
#
# db/verifier/oacx/admin은 각각 하나의 리포지토리로 통합 관리된다. DB는 자체
...
# ============================================================================
echo
ask "사이트 코드 (이동 태그로 사용 -- 이 트랙이 현재 쓰는 이미지를 가리킴)" "$DEFAULT_SITE_TAG"
SITE_TAG="$REPLY"
...
echo "              : $MOVING_IMAGE (이동 태그, 이 트랙이 현재 쓰는 이미지)"
fi
echo "==========================================================="
}
