#!/usr/bin/env bash
# ============================================================================
# PostgreSQL 베이스 이미지 빌드 + servicetech2 레지스트리 push 스크립트 (bash)
#
...
echo "=============================================================="
echo " PostgreSQL 베이스 이미지 빌드 + servicetech2 레지스트리 등록"
echo "=============================================================="
...
# ---------- 2. PostgreSQL 버전 선택 ----------
echo
echo "PostgreSQL 버전을 선택하세요 (전부 계정/로그인 불필요, 공개 이미지):"
echo "  1) latest  (최신 안정 버전)"
echo "  2) 16      (안정 버전)"
echo "  3) 15      (구버전, 레거시 호환용)"
ask "번호 선택" "1"
case "$REPLY" in
  1) UPSTREAM_IMAGE="postgres:latest"; TAG="latest" ;;
  2) UPSTREAM_IMAGE="postgres:16";     TAG="16" ;;
  3) UPSTREAM_IMAGE="postgres:15";     TAG="15" ;;
