<#
.SYNOPSIS
  PostgreSQL 베이스 이미지 빌드 + servicetech2 레지스트리 push 스크립트 (PowerShell)

.DESCRIPTION
  지원 환경: Windows PowerShell 5.1+ / PowerShell 7+

  라이선스 주의사항
    - PostgreSQL은 완전 오픈소스(PostgreSQL License)이며 Docker Hub 공식 이미지
...
Write-Host "=============================================================="
Write-Host " PostgreSQL 베이스 이미지 빌드 + servicetech2 레지스트리 등록"
Write-Host "=============================================================="
...
Write-Host ""
Write-Host "PostgreSQL 버전을 선택하세요 (전부 계정/로그인 불필요, 공개 이미지):"
Write-Host "  1) latest  (최신 안정 버전)"
Write-Host "  2) 16      (안정 버전)"
Write-Host "  3) 15      (구버전, 레거시 호환용)"
$VerSel = Ask "번호 선택" "1"
switch ($VerSel) {
    "1" { $UpstreamImage = "postgres:latest"; $Tag = "latest" }
    "2" { $UpstreamImage = "postgres:16";     $Tag = "16" }
    "3" { $UpstreamImage = "postgres:15";     $Tag = "15" }
