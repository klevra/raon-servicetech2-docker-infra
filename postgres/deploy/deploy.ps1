<#
.SYNOPSIS
  PostgreSQL 테스트 인스턴스 배포 스크립트 (PowerShell) — servicetech2 레지스트리 기반

.DESCRIPTION
  지원 환경: Windows PowerShell 5.1+ / PowerShell 7+
  전제조건 : oracle/registry/setup-registry.ps1, postgres/base/build-and-push.ps1
...
Write-Host "=============================================================="
Write-Host " PostgreSQL 테스트 인스턴스 배포 (servicetech2 레지스트리 기반)"
...
Write-Host ""
Write-Host "DB 종류를 선택하세요 (현재는 PostgreSQL만 지원):"
Write-Host "  1) PostgreSQL"
$dbSel = Ask "번호 선택" "1"
if ($dbSel -ne "1") {
    Write-Err2 "현재는 PostgreSQL만 지원합니다."
    exit 1
}
$DbKind = "postgres"
...
if ($LASTEXITCODE -ne 0) {
    Write-Err2 "이미지를 가져오지 못했습니다. 먼저 postgres/base/build-and-push.ps1 로 '$Tag' 태그를 등록하세요."
    exit 1
}

$DeployImage = "servicetech2/postgres-deploy:$Tag"
...
Write-Host ""
$ContainerName = Ask "컨테이너 이름" "postgres-$Tag-deploy"
...
Write-Host ""
Write-Warn2 "postgres 비밀번호는 화면에 표시되지 않으며, 어떤 파일에도 저장하지 않습니다."
$DbPassword = Ask-Secret "postgres 초기 비밀번호 (비우면 랜덤 비밀번호 자동 생성)"
$GeneratedPw = $false
...
Write-Host ""
Write-Info "postgres는 관리자(슈퍼유저) 계정입니다. 애플리케이션에서 쓸 별도 계정을 만들고 싶다면 아래에서 생성하세요."
Write-Info "(PostgreSQL 공식 이미지에는 보조 계정 자동 생성 기능이 없어, CREATE USER/GRANT SQL을 자동 생성해 주입합니다)"
$AppUser = ""
$AppPassword = ""
$AppGeneratedPw = $false
if (Confirm "애플리케이션 계정을 생성할까요? (생성 시 '$DbName' DB에 전체 권한 부여)") {
    $rawAppUser = Ask "애플리케이션 계정 이름" "appuser"
    # PostgreSQL 대소문자 함정 방지: 항상 소문자로 통일 (스크립트 상단 설명 참고)
    $AppUser = $rawAppUser.ToLowerInvariant()
...
"@
        [System.IO.File]::WriteAllText((Join-Path $StagingDir "05_app_account.sql"), $appSql, [System.Text.UTF8Encoding]::new($false))
        Write-Ok "애플리케이션 계정 생성 SQL을 스테이징했습니다 (05_ 접두어, DDL보다 먼저 실행됨)"
    }
    if ($DdlDir) {
    ...
    $SetupMount = $true
    Write-Warn2 "PostgreSQL의 /docker-entrypoint-initdb.d/ 자동 실행은 데이터 볼륨이 비어있는 최초 기동 시에만 동작합니다 (이 프로젝트는 항상 휘발성이라 매번 최초 기동입니다)."
}

Write-Host ""
$ListenerPort = Ask "포트" "5432"
...
Write-Both " ---------------------------------------------------------"
Write-Both " [관리자] 계정 : postgres"
Write-Both " [관리자] URL  : jdbc:postgresql://localhost:$ListenerPort/$DbName"
if ($AppUser) {
    Write-Both " ---------------------------------------------------------"
    Write-Both " [앱]   계정   : $AppUser  ($DbName DB에 전체 권한)"
    Write-Both " [앱]   URL    : jdbc:postgresql://localhost:$ListenerPort/$DbName"
} else {
    Write-Both " ---------------------------------------------------------"
    Write-Both " [앱]   계정   : (생성 안 함, postgres로만 접속)"
}
Write-Both " ---------------------------------------------------------"
...
if ($GeneratedPw) { Write-Both " 생성된 비밀번호(postgres) : $DbPassword  ⚠ 다시 표시되지 않으니 지금 저장하세요" }
...
$RunArgs = @("-d", "--name", $ContainerName, "-p", "${ListenerPort}:5432")
...
Write-Both " Username   : postgres"
Write-Both " JDBC URL   : jdbc:postgresql://localhost:$ListenerPort/$DbName"
if ($GeneratedPw) {
    Write-Both " 접속 예시  : PGPASSWORD=$DbPassword psql -h 127.0.0.1 -p $ListenerPort -U postgres -d $DbName"
} else {
    Write-Both " 접속 예시  : PGPASSWORD=<입력한 비밀번호> psql -h 127.0.0.1 -p $ListenerPort -U postgres -d $DbName"
}
if ($AppUser) {
    Write-Both " ---------------------------- [앱] -----------------------------"
    ...
    Write-Both " JDBC URL   : jdbc:postgresql://localhost:$ListenerPort/$DbName"
    if ($AppGeneratedPw) {
        Write-Both " 접속 예시  : PGPASSWORD=$AppPassword psql -h 127.0.0.1 -p $ListenerPort -U $AppUser -d $DbName"
    } else {
        Write-Both " 접속 예시  : PGPASSWORD=<입력한 비밀번호> psql -h 127.0.0.1 -p $ListenerPort -U $AppUser -d $DbName"
    }
}
Write-Both "==========================================================="
if ($LogFile) {
