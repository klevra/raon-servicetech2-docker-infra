<#
.SYNOPSIS
  MySQL 테스트 인스턴스 배포 스크립트 (PowerShell) — servicetech2 레지스트리 기반

.DESCRIPTION
  지원 환경: Windows PowerShell 5.1+ / PowerShell 7+
  전제조건 : oracle/registry/setup-registry.ps1, mysql/base/build-and-push.ps1
...
Write-Host "=============================================================="
Write-Host " MySQL 테스트 인스턴스 배포 (servicetech2 레지스트리 기반)"
...
if ($LASTEXITCODE -ne 0) {
    Write-Err2 "이미지를 가져오지 못했습니다. 먼저 mysql/base/build-and-push.ps1 로 '$Tag' 태그를 등록하세요."
    exit 1
}
...
Write-Info "(MySQL 공식 이미지 기능으로 생성 -- 별도 SQL 없이 지정한 DB에 자동으로 전체 권한 부여됨)"
$AppUser = ""
$AppPassword = ""
...
if ($DdlDir -or $DmlDir) {
    ...
    $SetupMount = $true
    Write-Warn2 "MySQL의 /docker-entrypoint-initdb.d/ 자동 실행은 데이터 볼륨이 비어있는 최초 기동 시에만 동작합니다 (이 프로젝트는 항상 휘발성이라 매번 최초 기동입니다)."
}
...
Write-Both " ---------------------------------------------------------"
Write-Both " 접속 IP 제한  : 없음 (0.0.0.0 바인딩, MYSQL_USER는 '%'(모든 host)로 생성됨)"
...
if ($GeneratedPw) {
    ...
    Write-Both " 접속 예시  : mysql -h 127.0.0.1 -P $ListenerPort -u root -p<입력한 비밀번호> $DbName"
}
if ($AppUser) {
    Write-Both " ---------------------------- [앱] -----------------------------"
    ...
    if ($AppGeneratedPw) {
        ...
        Write-Both " 접속 예시  : mysql -h 127.0.0.1 -P $ListenerPort -u $AppUser -p<입력한 비밀번호> $DbName"
    }
}
Write-Both "==========================================================="
if ($LogFile) {
